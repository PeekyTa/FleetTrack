...
export const analyticsService = {
  async getDeviceActivity(period: string = 'week') {
    ...
    // Generate time-series data for device activity
    const now = new Date();
    ...
    for (let i = 0; i < hours; i++) {
      ...
      // Simulate slight variation based on real counts
  ...
  async getDistanceStats() {
    // Approximate distance from location history
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      // Rough estimate: each location pair ≈ 0.5-2km movement
      results.push({
